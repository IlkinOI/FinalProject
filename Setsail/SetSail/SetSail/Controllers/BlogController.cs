...
namespace SetSail.Controllers
{
    public class BlogController : Controller
    {
        ...
        public ActionResult BlogDetailsIndex(int id)  // Blog Details Page //
        {
            ...
            bldet.User = db.Users.Include("UserSocials").Include("Blogs").FirstOrDefault(u=>u.Id==bldet.Blog.UserId);
            bldet.BlogComments = db.BlogComments.Include("User").Include("Blog").Where(bc=>bc.BlogId==id).ToList();
            bldet.UserSocials = db.UserSocials.Include("User").Where(s => s.UserId == bldet.User.Id).ToList();
        ...
        public ActionResult BlogComment(VmBlogDetails vmbc)
        {
                ...
                string.IsNullOrEmpty(vmbc.Email))
            {
                ...
                return RedirectToAction("BlogDetailsIndex", "Blog", new { id = vmbc.Blog.Id });
            }
        ...
        public ActionResult Subscribe(VmBlogDetails details)
        {
            ...
            {
                Session["Empty"] = true;
                return RedirectToAction("BlogDetailsIndex", "Blog", new { id = details.Blog.Id });
            }
            Subscription Subscribe = new Subscription();
            Subscribe.Email = details.SEmail;
            Subscribe.Fullname = details.SFullname;
